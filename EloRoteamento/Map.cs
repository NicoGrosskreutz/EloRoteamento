...
namespace EloRoteamento
{
    public partial class Map : Form
    {
        ...
        /// <summary>
        /// AUXILIAR DO ARRAY CONTENDO OS PARAMETROS
        /// </summary>
        string[] aux;
        ...
        private bool LoadLocalization(string[] args)
        {
            try
            {
                if (args.Length > 0)
                {
                    ...
                    this.Points = new List<Reference>();
                    this.Points.Clear();
                    ...
                    if (this.Points.Count == 0 && marcas.Count > 0)
                    {
                        gMap.Position = marcas.First() != null ? marcas.First().Point : new PointLatLng(-14.2350, -51.9253);
                        gMap.Zoom = 14;
                    }
                }
                else
        ...
        private void LoadTable()
        {
            ...
            grid.Columns["DataHora"].Width = grid.Width;
        }
        ...
        private void cbFTRVND_CheckedChanged(object sender, EventArgs e)
        {
            if (cbFTRVND.Checked == true)
            {
                if (aux.Length > 0)
                {
                    string strArray = "locais=";
                    ...
                    aux.ToList().ForEach(a =>
                    {
                        if (a.ToLower().StartsWith("locais="))
                        {
                            string[] positions = a.Replace("locais=", "").Split(';');
